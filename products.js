...
async function loadCatalog() {
  const [products, designs, dtf, sizePricing, itemTypes] = await Promise.all([
    ...
    fetch("data/dtf.json").then((r) => r.json()),
    fetch("data/sizePricing.json").then((r) => r.json()),
    fetch("data/itemTypes.json").then((r) => r.json()),
  ]);
...
function isArtwork(path) {
  return typeof path === "string" && path.startsWith("assets/dtf/");
}
